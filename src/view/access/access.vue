        ...
        <Card>
          ...
          <div class="access-user-con access-change-access-con">
            ...
            <Col span="17" class="padding-left-10">
              <Row type="flex" justify="center" align="middle" class="access-change-access-con-row">
                <p>
                  您可以通过更换测试用户账号然后观察该页面按钮的变化<br><br> 自定义权限标签：
      ...
      <Col :md="12" :lg="10" style="padding:0 10px 10px 0">
        <Card>
          ...
          <Row>
            ...
            <b>{{ rolesTypes }}</b>
            <p>
              自定义角色权限标签：v-hasRole 示例：{{exampleRole}}
            </p>

            <div class="sorry">
              <img src="@/assets/images/sorry.png" width="215">
              <span class="text">抱歉，请获取完整版</span>
              <Button to="http://xpay.exrick.cn/pay?xboot" target="_blank" type="error" icon="md-paper-plane">立即获取
              </Button>
            </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      avatorPath() {
        return store.state.user.avatorImgPath;
      }
    },
    methods: {
      initMeta() {
