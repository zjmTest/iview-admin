      ...
      <Form ref="userForm" :model="userForm" :label-width="70" :rules="userFormValidate">
        ...
        <FormItem label="角色分配" prop="roles">
          <Select v-model="userForm.roles" multiple>
            <Option v-for="item in roleList" :value="item.role" :key="item.role" :label="item.name">
...
<script>
  ...
  import {initDepartment, loadDepartment} from '@/api/system/department'
  ...
  export default {
    ...
    methods: {
      ...
      loadDataTree(item, callback) {
        loadDepartment(item.deptId).then(res => {
          if (res.success === true) {
            res.data.forEach(function (e) {
              if (e.isParent) {
                e.loading = false;
      ...
      selectTree(v) {
        if (v.length > 0) {
          ...
          this.userForm.departmentId = data.deptId;
